// Fragment Shader del cielo + esfera + plano
// En este shader construimos rayos por píxel, pero sólo para mostrar un cielo + esfera
// + plano horizontal
...
// Intersección rayo-plano horizontal
// Recordar ecuación del plano: f(p) = p.n - c = 0
// En este caso, solo importa la componente 'y' con n = (0,1,0)
// Luego t = (c - o.y) / d.y
float hitFloor(vec3 o, vec3 d, float c){
    float t = -1.0;
    float denom = d.y;
    if (abs(denom) > EPSILON) { // evitar división por cero
        t = (c - o.y) / denom;
        if (t < 0.0) t = -1.0; // plano detrás del origen
    }
    return t;
}
...
void main(){
    ...
    float tSphere = hitSphere(rayOrigin, rayDir, SPHERE_CENTER(), SPHERE_RADIUS());
    float tFloor  = hitFloor(rayOrigin, rayDir, FLOOR_HEIGHT());

    HitInfo hit = makeNoHit();
    hit = updateHit(tSphere, OBJ_SPHERE, 0, hit);
    hit = updateHit(tFloor,  OBJ_PLANE,  0, hit);

    if (hit.type == OBJ_SPHERE) {
        color = vec3(1.0, 0.0, 0.0); // Rojo para la esfera
    } else if (hit.type == OBJ_PLANE) {
        color = colorTablero(rayOrigin + rayDir * hit.t);
    } else {
