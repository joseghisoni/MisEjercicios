...
// Parámetros de iluminación (conectados desde ejercicio.js)
uniform vec3 lightdir;      // dirección de la luz direccional (mundo)
uniform float alpha;        // exponente especular (shininess)
const float EPS = 0.001;    // sesgo para evitar auto-intersecciones en rayos de sombra
...
// Sombreado Blinn-Phong simplificado inspirado en el TP anterior
// p: punto de intersección en mundo
// n: normal en el punto (unidad)
// Kd: color difuso base
...
float hitPlane(Ray ray, Plane plane){
    ...
    if (abs(denom) > EPS) { // evitar división por cero
...
float computeShadow(vec3 p, vec3 n, int objType, Plane plane) {
    vec3 origin = p + n * EPS; // evitar auto-ocultamiento
...
vec3 computeLighting(vec3 p, vec3 n, vec3 v, vec3 Kd, vec3 Ks, int objType, Plane plane) {
 ...
 (Kd + Ks *( (pow(cosphiblinn, alpha) / max(costheta, EPS))) );
...
vec3 traceRay(Ray ray) {
    ...
    for (int depth = 0; depth <= MAX_TRANSPARENCY_DEPTH; ++depth) {
        ...
        currentRay = makeRay(p + currentRay.dir * EPS, currentRay.dir);
    }
